# EVPN Type-2 (1 RR, 2 VTEPs)

This example builds one route reflector and two VTEPs.
`lab.yaml` uses IPv4 loopbacks and point-to-point underlay links for BGP reachability, then stretches a single L2 segment with EVPN type-2 over VXLAN.
...
- EVPN type-2 advertises the local MAC/IP bindings for the two host namespaces.
